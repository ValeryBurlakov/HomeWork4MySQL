...
-- TASK ...Подсчитать общее количество лайков, которые получили пользователи младше 12 лет.
SELECT COUNT(likes.id) AS total_likes_for_media_12_year
FROM likes 
JOIN media ON likes.media_id = media.id
JOIN users ON media.user_id = users.id
JOIN `profiles` ON users.id = `profiles`.user_id
WHERE `profiles`.birthday > (NOW() - INTERVAL 12 YEAR);
-- user_id 7 2014-07-31 'Austyn' 'Braun' media.id 7, 0 likes
-- user_id 2 2011-12-20 'Frederik', 'Upton' media.id 2 and 20, 4 likes


-- TASK ...Определить кто больше поставил лайков (всего): мужчины или женщины.
...
-- TASK ...Вывести всех пользователей, которые не отправляли сообщения.
